...
public class B extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Void> {
    ...
    static class ContohLoader extends AsyncTaskLoader<Void> {
        static WeakReference<B> mActivity;
        int mCounter = 0;

        public ContohLoader(@NonNull Context context, int n, B main) {
            super(context);
            mCounter = n;
            mActivity = new WeakReference<B>(main);
        }
        @Nullable
        @Override
